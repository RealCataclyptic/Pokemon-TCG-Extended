...
; has only Mewtwo Pokemon cards (running a Mewtwo mill deck).
InitAITurnVars:
...
; if the card is Mewtwo, it means the Player
...
; check if Player is running Mewtwo-only deck,
; if so, set wAIBarrierFlagCounter flag.
	...
	cp16 MEWTWO
	jr nz, .reset_1
	farcall CheckIfPlayerHasPokemonOtherThanMewtwo
	jr nc, .set_flag
.reset_1
