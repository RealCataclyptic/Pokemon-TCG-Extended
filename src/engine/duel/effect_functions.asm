...
;---------------------------------------------------------------------------------
; NEW EFFECT FUNCTIONS START NOW
;---------------------------------------------------------------------------------

SynchroCheckEffect: 
	ld e, PLAY_AREA_ARENA
	call GetPlayAreaCardAttachedEnergies ; gets your energies
	ld d, a ; loads this to variable a
	call SwapTurn
	call GetPlayAreaCardAttachedEnergies ; gets opp's energies
	call SwapTurn
	cp d ; does add c (your energies)
	ret nz ; if energies are not equal, return. 
	ld a, 50 ; does more damage if energies are equal, NOT energy cards.
	jp AddToDamage; jp to save space

SynchoAIEffect: ; AI only understands that equal energies = bonus damage. Does not understand that attaching energy will do it. 
	call SynchroCheckEffect
	jp SetDefiniteAIDamage

EnergyCrushEffect: 
	ld e, PLAY_AREA_ARENA
	call SwapTurn
	call GetPlayAreaCardAttachedEnergies 	; gets opponent energies
	ld d, a 				; loads this to variable d
	call SwapTurn
	call GetPlayAreaCardAttachedEnergies 	; gets your energies
	sub d 					; subs d from a (your energy - opponents energy)
	ret c 					; if underflow, return
	ret z 					; if same number, return to save cycles
	call ATimes10 				; otherwise multiply by 10
	jp AddToDamage
						; Can put literally anything here as an effect. Update this with energy synchro. 

EnergyCrushAIEffect: 
	call EnergyCrushEffect
	jp SetDefiniteAIDamage

; chooses a card at random from the opponents hand and moves it to the discard pile. return carry if there are no cards to discard
; returns deck index of discarded card in a
Discard1RandomCardFromOpponentsHand:
	call Get1RandomCardFromOpponentsHand
  	ret c
; could use MoveHandCardToDiscardPile here, but the check to avoid anything other than CARD_LOCATION_HAND is redundant, it is already done in CreateHandCardList
  	call SwapTurn
...
; chooses a card at random from the opponents hand. Return carry if there are no cards. Returns deck index of selected card or $ff in a
...
; deck index is already in a. Show respective card from the opposing player's deck
  	call SwapTurn
...
OpponentMills1Effect:
	ld a, 1	; alter this for any number
	ldh [hTemp_ffa0], a
	call OpponentDeck_DiscardXCardsEffect
	ret

EveryoneMills10Effect: ; Probably need AI logic if it uses this attack. Check self deck <10 cards, check opponent deck >10 cards, no use.
	ld a, 10	; alter this for any number
	ldh [hTemp_ffa0], a
	call OpponentDeck_DiscardXCardsEffect
	call SwapTurn
	call OpponentDeck_DiscardXCardsEffect
	call SwapTurn
	ldtx hl, EveryoneMilledText ; have to delete the ldtx hl in DiscardXCards or else it loads that message anyway
	call DrawWideTextBox_PrintText ; no need for pop or push hl because the text has the number already in.
	ret

HandleOnAttackEffects:: ; Main function in Home > Duel
	ld a, [wTempNonTurnDuelistCardID]
	cp PROFESSOR_OAK ; nonsense card, can be changed for actual effects
	ret nz
	call SwapTurn
	call PoisonEffect ; poisons opp
	call SwapTurn
	ret

CantAttachEnergyEffect:
	call SwapTurn
	ldh a, [hTemp_ffa0]
	ld a, DUELVARS_ARENA_CARD_FLAGS
	call GetTurnDuelistVariable
	set CANT_ATTACH_ENERGY_THIS_PKMN_F, [hl]
	call SwapTurn 
	ret

Do10XCardsInOppHandEffect:
	call SwapTurn
	call Do10XCardsInHandEffect
	call SwapTurn
	ret
...
AI_Do10XCardsInHandEffect:
	call Do10XCardsInHandEffect
	jp SetDefiniteAIDamage

AI_Do10XCardsInOppHandEffect:
	call Do10XCardsInOppHandEffect
	jp SetDefiniteAIDamage

CalcAllRCInPlayAreaEffect:
; sets attack damage to 10x the number of {C} symbols
; in the Retreat Costs of all of the turn holder's Pok??mon.
; preserves de
    	ld a, DUELVARS_NUMBER_OF_POKEMON_IN_PLAY_AREA
   	call GetTurnDuelistVariable
    	ld b, a
    	ld l, DUELVARS_ARENA_CARD
   	; hl is now set to the first duel variable that holds the deck indices of the turn holder's play area Pok??mon
   	ld c, 0 ; initial Retreat Cost counter
   	ld e, c ; PLAY_AREA_ARENA

; loop through each play area Pok??mon, adding its Retreat Cost to c
.loop_play_area
   	ld a, [hli]
...
CalcAllEnergyOnFieldEffect:
	call CalcAllEnergyInYourPlayAreaEffect 		; Do the calc for all your energies first
	call SwapTurn					; Swap to the opponent
	ld c, 0 					; set c to 0 at beginning
	ld a, DUELVARS_NUMBER_OF_POKEMON_IN_PLAY_AREA
	call GetTurnDuelistVariable
	ld d, a
	ld e, c
.loop_play_area
	call GetPlayAreaCardAttachedEnergies		; Get its attached energies
	add c						; add c
   	ld c, a						; store a to c
	inc e						; increase position of pokemon location in numeral form
	dec d						; decrease by 1 the number of pokemon found in DUELVARS_NUMBER_OF_POKEMON_IN_PLAY_AREA
	jr nz, .loop_play_area				; if it hits 0, continue. Otherwise, go back in the loop
	ld a, c						; load c to a
	call ATimes10										
	call AddToDamage				; Add to damage instead of Setdefinitedamage
	jp SwapTurn					; Swap back you to end it

AI_CalcAllEnergyOnFieldEffect:
	call CalcAllEnergyOnFieldEffect
	jp SetDefiniteAIDamage

CalcAllEnergyInYourPlayAreaEffect:
	ld c, 0 					; set c to 0 at beginning
	ld a, DUELVARS_NUMBER_OF_POKEMON_IN_PLAY_AREA
	call GetTurnDuelistVariable
	ld d, a
...
AI_CalcAllEnergyInYourPlayAreaEffect:
	call CalcAllEnergyInYourPlayAreaEffect
	jp SetDefiniteAIDamage


ExplosiveEvolutionEffect:
	call CheckEvol
	call Evolution_PlayerSelectEffect
...
MorePrizesEffect:
	call CountPrizes
	ld b, a ; turn duelists prizes to a
	call SwapTurn
	call CountPrizes
	call SwapTurn
	cp b
	jr c, .MorePrizesThanOpp
	ret ; less prizes
.MorePrizesThanOpp
	ld a, 50
	call AddToDamage
...
AttachEnergyCardInHandSelectEffect:
	ld a, $ff
	ldh [hTemp_ffa0], a
...
.select_card
	bank1call DisplayCardList
	; jr c, .try_cancel ; Maybe do this?
	call GetCardIDFromDeckIndex
	call GetCardType
	cp TYPE_ENERGY_DOUBLE_COLORLESS
	jr nc, .select_card ; not a Basic Energy card

	ldh a, [hTempCardIndex_ff98] ; Energy card selected
	ldh [hTemp_ffa0], a
	call EmptyScreen
...
.play_sfx
	call PlaySFX_InvalidChoice ; invalid choice sfx
	jr .select_card
...
;AI_AttachEnergyCardInHandEffect: ; BROKEN
	;farcall CreateEnergyCardListFromHand
	;ld hl, wDuelTempList
;.loop_hand
	;ld a, [hli]
...
CheckSelfDamage_Effect:
	ld e, PLAY_AREA_ARENA
	call GetCardDamageAndMaxHP
	cp 10
	ldtx hl, CantUseNoDamageText
	ret

CheckSelfDamage_AI_Effect:
	call GetTurnDuelistVariable
	ld e, PLAY_AREA_ARENA
	call GetCardDamageAndMaxHP
	cp 10
